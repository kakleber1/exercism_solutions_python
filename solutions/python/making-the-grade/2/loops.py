...
def letter_grades(highest):
    ...
    grades = []
    thresholds = []
    for grade in range(41, highest):
        grades.append(grade)
    interval = round(len(grades) / 4)
    for grade in range (41, highest, interval):
        thresholds.append(grade)
    return thresholds
...
def student_ranking(student_scores, student_names):
    ...
    score_name_pairs = []
    counter = 0
    for item in enumerate(student_scores):
        score_name_pairs.append(f"{counter + 1}. {student_names[counter]}: {student_scores[counter]}")
        counter += 1
