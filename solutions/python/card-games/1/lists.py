...
def approx_average_is_average(hand):
    ...
    first_last_avg = float((hand[0] + hand[-1]) / 2)
    median = hand[len(hand) // 2]
    if actual_average == first_last_avg or actual_average == median:
        return True
    return False
...
def average_even_is_average_odd(hand):
    ...
    """
    sum_odds = sum(hand[::2])
    sum_evens = sum(hand[1::2])
    if float(sum_odds / len(hand[::2])) == float(sum_evens / len(hand[1::2])):
        return True
    return False
...
def maybe_double_last(hand):
    ...
    """
    if hand[-1] == 11:
        new_value = int(hand[-1]) * 2
        new_hand = hand[:-1] + [new_value]
        return new_hand
    return hand